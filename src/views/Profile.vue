					...
					<div class="mb-3">
						...
						<div class="input-group mb-3">
							<input 
								...
								v-on:keyup.enter="addExistingDomain"
								type="text" 
...
export default {
	...
	methods: {
		...
		async addExistingDomain() {
			this.existingDomain = this.existingDomain.toLowerCase()
			const intfc = new ethers.utils.Interface(tldAbi)
			const contract = new ethers.Contract(this.getTldAddress, intfc, this.signer)
			const checkDomainHolder = await contract.getDomainHolder(this.existingDomain)
			if (String(checkDomainHolder).toLowerCase() === String(this.address).toLowerCase()) {
				this.addDomainManually(this.existingDomain+this.getTldName)
