		...
		<div class="row mt-5">
			<div class="col-md-4 offset-md-4">
				<div class="input-group mb-3">
					<input v-model="receiver" type="text" class="form-control" placeholder="Recipient's username" aria-label="Recipient's username" aria-describedby="basic-addon2">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="row mb-3 mt-4">
						...
						<div class="col-sm-9">
							<span>{{ domainLowerCase }}{{ getTldName }}</span>
					...
					<div class="row mb-3 mt-2" v-if="receiverAddress">
						...
						<div class="col-sm-9">
							...
							<span class="domain-error" v-if="String(receiverAddress).toLowerCase() == String(address).toLowerCase()"
...
export default {
	...
	computed: {
		...
		domainLowerCase() {
			if (this.receiver) {
				return this.receiver.replace(".🥁", "").toLowerCase()
			}
		...
		notValid() {
			if (!this.receiver) {
				return true
			} else if (this.domainLowerCase.includes('.')) {
				return true
	...
	methods: {
		...
		async validateDomainName() {
			...
			try {
				if (this.domainLowerCase && !this.domainLowerCase.includes('.')) {
					for (let netId in tldsJson) {
						// iterate through different chains
						if (tldsJson[netId][this.getTldName]) {
							...
							const tldContractRead = new ethers.Contract(
								tldsJson[netId][this.getTldName],
								intfc,
								fProvider,
							)
							// fetch domain holder
							const recDomainHolder = await tldContractRead.getDomainHolder(this.domainLowerCase)
